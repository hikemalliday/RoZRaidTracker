...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if not options["noinput"] or not options["no-input"]:
            confirm = input(
                "WARNING: You are about to perform a migration from quarm items to default items. This will first clear ALL default items, and then perform migration. Are you sure you want to continue? (yes/no): "
            )
